...
app.get("/fetchBranches", async (req, res) => {
  try {
    const { data, error } = await supabase.from("branch_new").select("*");
...
async function getColleges(formData) {
  ...
  try {
    // First query
    const { data: mhtData, error: mhtError } = await supabase.rpc(
      "pcm_preference_list_mht",
      {
...
function college_filter_by_branch_category(colleges, branch_cat) {
  return colleges.filter((element) =>
    branch_cat.includes(element.branch_category)
  );
}
...
function college_filter(colleges, formData) {
  ...
  if (formData.selected_branches.length == 0) {
    if (formData.branchCategories[0] != "All") {
      colleges = college_filter_by_branch_category(
        colleges,
        formData.branchCategories
      );
    }
  } else {
    colleges = college_filter_by_branch_category(
    ...
    colleges = college_fillter_by_selected_branch(
      colleges,
...
app.post("/College_list", async (req, res) => {
  ...
  try {
    ...
    if (
      central_object.percentile < 80 &&
      formData.branchCategories[0] != "All"
    ) {
      formData.branchCategories.push("COMP");
      formData.branchCategories.push("COMPAI");
    }

    new_data_of_student.selected_branches_code = await getSelectedBranchCode(
      formData.selected_branches
    );
